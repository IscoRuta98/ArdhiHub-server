...
def buy_xrp(amount_usd, price, venue):
    ...
    logger.info(
        f"BUY on {venue}: Price={price:.4f}, USD={amount_usd:.2f}, XRP={xrp_bought:.4f}"
    )
...
def sell_xrp(amount_xrp, price, venue):
    ...
    trade_history.append(trade_details)
    logger.info(
        f"SELL on {venue}: Price={price:.4f}, XRP={amount_xrp:.4f}, USD={usd_received:.2f}"
    )
...
def main():
    ...
    try:
        while datetime.datetime.now() < trading_end:
            ...
            if (
                price_venue1 < price_venue2
                and (price_venue2 - price_venue1) / price_venue1 >= threshold
            ):
                ...
                price_venue2 < price_venue1
                and (price_venue1 - price_venue2) / price_venue2 >= threshold
            ):
